	...
	public async showQuickPick(
		...
		const pickItems = options.map((option) => {
			return {
				...
				description: option.filePath
					? path.basename(option.filePath)
					: "",
				detail: option.projectName
	...
	public async showQuickPickWithRecentlyUsed(
		...
		const pickItems = adjustedOptions.map((option) => {
			...
			if (isFromActiveEditor(option) && option.filePath) {
				adjustedDetail.push(
					`$(file-text) active editor (${path.basename(option.filePath)})`,
				);
			}
