...
function convertToJavaProcess(
	...
	if (JAVA_PATTERN.test(simpleName) && args) {
		...
		if (match && match.length) {
			...
			const commandPath =
				path.basename(binPath) === "bin"
					? path.join(
							path.basename(path.dirname(binPath)),
							"bin",
							exeName,
						)
					: exeName;
...
export async function pickJavaProcess(): Promise<IJavaProcess | undefined> {
	...
	if (!javaProcesses.length) {
		throw new Error(
			"Process picker: Cannot find any debuggable Java process. Please make sure to use vmArgs like " +
...
export async function resolveJavaProcess(
	pid: number,
): Promise<IJavaProcess | undefined> {
