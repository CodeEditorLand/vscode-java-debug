...
export function registerVariableMenuCommands(
	context: vscode.ExtensionContext,
): void {
	vscode.workspace.onDidChangeConfiguration((event) => {
		if (
			event.affectsConfiguration("java.debug.settings") ||
			event.affectsConfiguration("debug.autoExpandLazyVariables")
		) {
			updateContextKeys();
		}
	});
	...
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand("java.debug.variables.showHex", () =>
			updateVariableFormatter("showHex", true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.notShowHex",
			() => updateVariableFormatter("showHex", false),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.showQualifiedNames",
			() => updateVariableFormatter("showQualifiedNames", true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.notShowQualifiedNames",
			() => updateVariableFormatter("showQualifiedNames", false),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.showStaticVariables",
			() => updateVariableFormatter("showStaticVariables", true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.notShowStaticVariables",
			() => updateVariableFormatter("showStaticVariables", false),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.showLogicalStructure",
			() => updateVariableFormatter("showLogicalStructure", true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.notShowLogicalStructure",
			() => updateVariableFormatter("showLogicalStructure", false),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.showToString",
			() => updateVariableFormatter("showToString", true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.notShowToString",
			() => updateVariableFormatter("showToString", false),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.autoExpandLazyVariables",
			() => toggleLazyVariableSetting(true),
		),
	);
	context.subscriptions.push(
		instrumentOperationAsVsCodeCommand(
			"java.debug.variables.manualExpandLazyVariables",
			() => toggleLazyVariableSetting(false),
		),
	);
}
...
async function updateVariableFormatter(key: string, value: any) {
	...
	await debugSettingsRoot.update(
		key,
		value,
		getConfigurationTarget("java.debug", "settings"),
	);
	refreshVariableView();
}
...
async function toggleLazyVariableSetting(toggle: boolean) {
	...
	await debugSettingsRoot.update(
		...
		toggle,
		getConfigurationTarget("debug", "autoExpandLazyVariables"),
	);
	refreshVariableView();
}
...
function updateContextKeys() {
	...
	if (debugSettingsRoot) {
		vscode.commands.executeCommand(
			"setContext",
			"javadebug:showHex",
			debugSettingsRoot.showHex ? "on" : "off",
		);
		vscode.commands.executeCommand(
			"setContext",
			"javadebug:showLogicalStructure",
			debugSettingsRoot.showLogicalStructure ? "on" : "off",
		);
		vscode.commands.executeCommand(
			"setContext",
			"javadebug:showQualifiedNames",
			debugSettingsRoot.showQualifiedNames ? "on" : "off",
		);
		vscode.commands.executeCommand(
			"setContext",
			"javadebug:showStaticVariables",
			debugSettingsRoot.showStaticVariables ? "on" : "off",
		);
		vscode.commands.executeCommand(
			"setContext",
			"javadebug:showToString",
			debugSettingsRoot.showToString ? "on" : "off",
		);
	}
	...
	if (globalDebugRoot) {
		vscode.commands.executeCommand(
			...
			globalDebugRoot.autoExpandLazyVariables ? "on" : "off",
		);
	}
}
