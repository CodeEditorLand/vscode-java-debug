...
export class JavaInlineValuesProvider implements InlineValuesProvider {
	public async provideInlineValues(
		...
		const provideInlineValuesOperation = instrumentOperation(
			"provideInlineValues",
			async (operationId) => {
				const resolveInlineVariablesStep = instrumentOperationStep(
					operationId,
					"resolveInlineVariables",
					async () => {
						return <InlineVariable[]>await resolveInlineVariables({
							...
							stoppedLocation: codeConverter.asRange(
				...
				const resolveInlineValuesStep = instrumentOperationStep(
					operationId,
					"resolveInlineValues",
					async () => {
						...
						const unresolvedVariables: any[] = variables
							.filter(
								(variable) =>
									variable.kind === InlineKind.Evaluation,
							)
							.map((variable) => {
								return {
									expression:
						...
						sendInfo(operationId, {
							...
							inlineVariableLookupCount:
								variables.length - unresolvedVariables.length,
							inlineVariableEvaluationCount:
						...
						if (
							...
							const response =
								await debug.activeDebugSession.customRequest(
									"inlineValues",
									{
							...
							if (variable.kind === InlineKind.VariableLookup) {
								result.push(
									new InlineValueVariableLookup(
										protoConverter.asRange(variable.range),
										variable.name,
										true,
									),
								);
								...
								if (resolvedValue) {
									result.push(
										new InlineValueText(
											protoConverter.asRange(
												variable.range,
											),
											`${variable.name} = ${resolvedValue.value}`,
										),
									);
								} else {
									result.push(
