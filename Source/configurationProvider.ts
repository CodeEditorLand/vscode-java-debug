...
export class JavaDebugConfigurationProvider
	implements vscode.DebugConfigurationProvider
{
	...
	public provideDebugConfigurations(
		...
		const provideDebugConfigurationsHandler = instrumentOperation(
			"provideDebugConfigurations",
			(_operationId: string) => {
				return <Thenable<vscode.DebugConfiguration[]>>(
					this.provideDebugConfigurationsAsync(folder, token)
				);
			},
		);
	...
	public resolveDebugConfigurationWithSubstitutedVariables(
		...
		const resolveDebugConfigurationHandler = instrumentOperation(
			...
			(_operationId: string) => {
				try {
					...
					return this.resolveAndValidateDebugConfiguration(
						folder,
						config,
						token,
					);
		...
		return new Promise(async (resolve, _reject) => {
			...
			try {
				...
				const launchConfigs = mainClasses.map((item) => {
					return {
						...defaultLaunchConfig,
						name: this.constructLaunchConfigName(
							item.mainClass,
							cache,
						),
	...
	private mergePlatformProperties(
		config: vscode.DebugConfiguration,
	...
	private constructLaunchConfigName(
		mainClass: string,
	...
	private async resolveAndValidateDebugConfiguration(
		...
		const isConfigFromInternal =
			config.__origin ===
				"internal" /** in-memory configuration from debugger */ ||
		...
		let progressReporter = progressProvider.getProgressReporter(
			config.__progressId,
		);
		if (!progressReporter && config.__progressId) {
			return undefined;
		} else if (!progressReporter) {
			progressReporter = progressProvider.createProgressReporter(
		...
		try {
			...
			if (config.request === "launch") {
				const mainClassOption = await this.resolveAndValidateMainClass(
					folder && folder.uri,
					config,
					progressReporter,
				);
				...
				if (
					...
					config.__workspaceFolder !== folder
				) {
				...
				if (
					...
					const result = <any[]>(
						await lsPlugin.resolveClasspath(
							config.mainClass,
							config.projectName,
						)
					);
				...
				if (_.isEmpty(config.javaExec)) {
					config.javaExec = await lsPlugin.resolveJavaExecutable(
						config.mainClass,
						config.projectName,
					);
				} else {
					if (!fs.existsSync(config.javaExec)) {
						throw new utility.UserError({
							message:
								"Java executable file path cannot be accessed, please specify a valid path in the launch.json.",
							type: Type.USAGEERROR,
						});
					}
				}
				...
				if (
					!config.shortenCommandLine ||
					config.shortenCommandLine === "auto"
				) {
					config.shortenCommandLine = await getShortenApproachForCLI(
						config,
						targetJavaVersion,
					);
				}
				...
				if (
					...
					if (Number.isNaN(pid)) {
						vscode.window.showErrorMessage(
							`The processId config '${config.processId}' is not a valid process id.`,
						);
						return undefined;
					}
			...
			utility.showErrorMessageWithTroubleshooting(
				utility.convertErrorToMessage(ex),
			);
			return undefined;
		} finally {
	...
	private async resolvePath(
		...
		const pathVariables: string[] = [
			ClasspathVariable.Auto,
			ClasspathVariable.Runtime,
			ClasspathVariable.Test,
		];
		const containedVariables: string[] = pathArray.filter((cp: string) =>
			pathVariables.includes(cp),
		);
		...
		const response: any[] = <any[]>(
			await lsPlugin.resolveClasspath(mainClass, projectName, scope)
		);
		const resolvedPaths: string[] = isModulePath
			? response?.[0]
			: response?.[1];
		if (!resolvedPaths) {
	...
	private async filterExcluded(
		...
		for (const p of paths) {
			if (p.startsWith("!")) {
				...
				excludes.set(
					vscode.Uri.file(exclude).fsPath,
					this.isFilePath(exclude),
				);
				continue;
			}
	...
	private isEmptyConfig(config: vscode.DebugConfiguration): boolean {
		return (
			Object.keys(config).filter((key: string) => key !== "noDebug")
				.length === 0
		);
	}

	private async resolveAndValidateMainClass(
		...
		if (config.mainClass && !this.isFilePath(config.mainClass)) {
			...
			const containsExternalClasspaths =
				!_.isEmpty(config.classPaths) || !_.isEmpty(config.modulePaths);
			...
			if (progressReporter.isCancelled()) {
				...
				!validationResponse.mainClass.isValid ||
				!validationResponse.projectName.isValid
			) {
				return this.fixMainClass(
					folder,
					config,
					validationResponse,
					progressReporter,
	...
	private async resolveMainClass(
		...
		if (config.projectName) {
			...
			if (this.isFilePath(config.mainClass)) {
				const mainEntries = await lsPlugin.resolveMainMethod(
					vscode.Uri.file(config.mainClass),
				);
				if (progressReporter.isCancelled()) {
					return undefined;
			...
			return this.promptMainClassUnderProject(
				config.projectName,
				progressReporter,
				"Please select a main class you wan to run",
			);
		}
		...
		const hintMessage = currentFile
			...
			: "Please select a main class you want to run.";
		return this.promptMainClassUnderPath(
			undefined,
			progressReporter,
			hintMessage,
		);
	...
	private getValidationErrorMessage(
		error: lsPlugin.IValidationResult,
	): string {
		switch (error.kind) {
	...
	private async fixMainClass(
		...
		if (!validationResponse.projectName.isValid) {
			...
			const errorLog: Error = {
				name: "error",
				message: this.getValidationErrorMessage(
					validationResponse.projectName,
				),
			};
			setUserError(errorLog);
			sendError(errorLog);
		...
		if (
			...
			validationResponse.proposals.length
		) {
			progressReporter.hide(true);
	...
	private async persistMainClassOption(
		folder: vscode.Uri | undefined,
	...
	private async persistLaunchConfig(
		...
		const targetIndex: number = _.findIndex(rawConfigs, (config) =>
			_.isEqual(config, oldConfig),
		);
		if (targetIndex >= 0) {
	...
	private async promptMainClassUnderPath(
		folder: vscode.Uri | undefined,
		progressReporter: IProgressReporter,
		hintMessage?: string,
	): Promise<lsPlugin.IMainClassOption | undefined> {
	...
	private async promptMainClassUnderProject(
		projectName: string,
		progressReporter: IProgressReporter,
		hintMessage?: string,
	): Promise<lsPlugin.IMainClassOption | undefined> {
...
async function updateDebugSettings(event?: vscode.ConfigurationChangeEvent) {
	const debugSettingsRoot: vscode.WorkspaceConfiguration =
		vscode.workspace.getConfiguration("java.debug");
	...
	if (
		...
		Object.keys(debugSettingsRoot.settings).length
	) {
		try {
			const stepFilters = {
				skipClasses: await substituteFilterVariables(
					debugSettingsRoot.settings.stepping.skipClasses,
				),
				skipSynthetics:
					debugSettingsRoot.settings.stepping.skipSynthetics,
				skipStaticInitializers:
					debugSettingsRoot.settings.stepping.skipStaticInitializers,
				skipConstructors:
					debugSettingsRoot.settings.stepping.skipConstructors,
			};
			const exceptionFilters = {
				exceptionTypes:
					debugSettingsRoot.settings.exceptionBreakpoint
						.exceptionTypes,
				allowClasses:
					debugSettingsRoot.settings.exceptionBreakpoint.allowClasses,
				skipClasses: await substituteFilterVariables(
			...
			const settings = await commands.executeJavaLanguageServerCommand(
				...
				JSON.stringify({
					...
					limitOfVariablesPerJdwpRequest: Math.max(
						debugSettingsRoot.settings.jdwp
							.limitOfVariablesPerJdwpRequest,
						1,
					),
					jdwpRequestTimeout: Math.max(
						debugSettingsRoot.settings.jdwp.requestTimeout,
						100,
					),
					asyncJDWP,
				}),
...
function needsBuildWorkspace(): boolean {
	const javaConfig: vscode.WorkspaceConfiguration =
		vscode.workspace.getConfiguration("java");
