...
export class JavaTerminalLinkProvder
	implements TerminalLinkProvider<IJavaTerminalLink>
{
	/**
	...
	): ProviderResult<IJavaTerminalLink[]> {
	...
	public async handleTerminalLink(link: IJavaTerminalLink): Promise<void> {
		...
		if (uri) {
			...
			const fullyQualifiedName = link.methodName.substring(
				0,
