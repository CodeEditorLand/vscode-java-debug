...
export async function buildWorkspace(
	...
	const buildResult = await instrumentOperation(
		...
		async (operationId: string) => {
			...
			try {
				...
				status =
					err && err.code === CANCELLED_CODE
						? CompileWorkspaceStatus.CANCELLED
						: err;
			}
	...
	if (
		...
		buildResult.status === CompileWorkspaceStatus.CANCELLED
	) {
		return false;
...
async function handleBuildFailure(
	...
	if (!onBuildFailureProceed && err) {
		...
		if (
			errorDiagnostics &&
			err !== CompileWorkspaceStatus.GRADLE_BS_COMPILATION_ERROR
		) {
