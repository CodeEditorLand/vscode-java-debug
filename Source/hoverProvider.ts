...
class DebugHoverProvider implements Disposable {
	...
	constructor() {
		this.runHoverCommand = instrumentOperationAsVsCodeCommand(
			JAVA_HOVER_RUN_COMMAND,
			async (noDebug: boolean, uri: string, position: any) => {
				const element = await resolveElementAtSelection(
					uri,
					position.line,
					position.character,
				);
				...
				if (element && element.hasMainMethod) {
					...
					window.showErrorMessage(
						"The hovered element is not a main method.",
					);
				}
			},
		);
...
class InternalDebugHoverProvider implements HoverProvider {
	...
	private convertCommandToMarkdown(command: Command): string {
		return (
			`[${command.title}](command:${command.command}?` +
