...
export async function showInformationMessageWithTroubleshooting(
	message: ITroubleshootingMessage,
	...items: string[]
): Promise<string | undefined> {
...
export async function showWarningMessageWithTroubleshooting(
	message: ITroubleshootingMessage,
	...items: string[]
): Promise<string | undefined> {
...
export function getJavaExtensionAPI(
...
export function isGitBash(isIntegratedTerminal: boolean): boolean {
	const currentWindowsShellPath: string | undefined = isIntegratedTerminal
		? vscode.env.shell
		: vscode.workspace
...
export async function waitForStandardMode(
	...
	if (api && api.serverMode === ServerMode.LIGHTWEIGHT) {
		const answer = await vscode.window.showInformationMessage(
			"Run/Debug feature requires Java language server to run in Standard mode. " +
...
export async function searchMainMethods(
	uri?: vscode.Uri,
): Promise<IMainClassOption[]> {
	return resolveMainClass(uri);
}

export async function searchMainMethodsWithProgress(
	uri?: vscode.Uri,
): Promise<IMainClassOption[]> {
	try {
...
async function isImportingProjects(): Promise<boolean> {
	...
	if (
		...
		return (
			...
			(extension.isActive &&
				extension.exports?.serverMode === ServerMode.HYBRID)
		);
	}
...
function getJavaServerMode(): ServerMode {
	return (
		vscode.workspace.getConfiguration().get("java.server.launchMode") ||
		ServerMode.HYBRID
	);
}
...
export function launchJobNameByMainClass(
	mainClass: string,
	noDebug: boolean,
): string {
	return launchJobName(
		mainClass.substr(mainClass.lastIndexOf(".") + 1),
		noDebug,
	);
}
