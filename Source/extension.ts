...
import {
	...
	startDebugging,
} from "./debugCodeLensProvider";
import { initExpService } from "./experimentationService";
import {
	handleHotCodeReplaceCustomEvent,
...
function specifyProgramArguments(
	context: vscode.ExtensionContext,
): Thenable<string> {
...
async function runJavaFile(uri: vscode.Uri, noDebug: boolean) {
	...
	try {
		...
		if (
			!uri &&
			activeEditor &&
			_.endsWith(path.basename(activeEditor.document.fileName), ".java")
		) {
...
function findLaunchConfiguration(
	mainClass: string,
