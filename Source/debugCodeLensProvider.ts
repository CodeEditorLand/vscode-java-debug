...
export function initializeCodeLensProvider(
	...
	if (isActivatedByJavaFile() && isJavaExtEnabled()) {
		getJavaExtensionAPI().then((api) => {
			if (
				api &&
				(api.serverMode === ServerMode.LIGHTWEIGHT ||
					api.serverMode === ServerMode.HYBRID)
			) {
				api.onDidServerModeChange((mode: string) => {
					if (mode === ServerMode.STANDARD) {
						context.subscriptions.push(
							new DebugCodeLensContainer(),
						);
					}
				});
			} else {
...
class DebugCodeLensContainer implements vscode.Disposable {
	...
	constructor() {
		this.runCommand = instrumentOperationAsVsCodeCommand(
		...
		this.configurationEvent = vscode.workspace.onDidChangeConfiguration(
...
class DebugCodeLensProvider implements vscode.CodeLensProvider {
	public async provideCodeLenses(
		...
		try {
			const mainMethods: IMainMethod[] = await resolveMainMethod(
				document.uri,
				token,
			);
...
async function constructDebugConfig(
	...
	let debugConfig: vscode.DebugConfiguration | undefined = _.find(
		rawConfigs,
		(config) => {
			return (
				config.mainClass === mainClass &&
				_.toString(config.projectName) === _.toString(projectName)
			);
		},
	);
	...
	if (!debugConfig) {
		...
		if (
			...
			(await launchJsonExists(workspace))
		) {
			try {
...
async function launchJsonExists(workspace?: vscode.Uri): Promise<boolean> {
	...
	const results: vscode.Uri[] = await vscode.workspace.findFiles(
		".vscode/launch.json",
		"**/node_modules/**",
	);
...
export async function startDebugging(
	...
	const debugConfig: vscode.DebugConfiguration = await constructDebugConfig(
		mainClass,
		projectName,
		workspaceUri,
	);
