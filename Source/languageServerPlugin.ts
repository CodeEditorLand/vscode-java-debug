...
export async function resolveMainMethod(
	uri: vscode.Uri,
	token?: vscode.CancellationToken,
): Promise<IMainMethod[]> {
	if (token) {
		return <IMainMethod[]>(
			await commands.executeJavaLanguageServerCommand(
...
export function resolveClasspath(
	...
	return commands.executeJavaLanguageServerCommand(
		commands.JAVA_RESOLVE_CLASSPATH,
		mainClass,
		projectName,
		scope,
	);
...
export function resolveMainClass(
	workspaceUri?: vscode.Uri,
): Promise<IMainClassOption[]> {
	if (workspaceUri) {
...
export function resolveMainClassFromProject(
	projectName: string,
): Promise<IMainClassOption[]> {
	return <Promise<IMainClassOption[]>>(
		commands.executeJavaLanguageServerCommand(
...
export function inferLaunchCommandLength(
...
export function resolveElementAtSelection(
	uri: string,
	line: number,
	character: number,
): Promise<any> {
	return <Promise<any>>(
		commands.executeJavaLanguageServerCommand(
			commands.JAVA_RESOLVE_ELEMENT_AT_SELECTION,
			uri,
			line,
			character,
		)
...
export function resolveBuildFiles(): Promise<string[]> {
	return <Promise<string[]>>(
		commands.executeJavaLanguageServerCommand(
			commands.JAVA_RESOLVE_BUILD_FILES,
		)
	);
}
...
export function resolveJavaExecutable(mainClass: string, projectName: string) {
	return commands.executeJavaLanguageServerCommand(
		commands.JAVA_RESOLVE_JAVAEXECUTABLE,
		mainClass,
		projectName,
	);
}
...
export async function resolveClassFilters(
	patterns: string[],
): Promise<string[]> {
	return <string[]>(
		await commands.executeJavaLanguageServerCommand(
			commands.JAVA_RESOLVE_CLASSFILTERS,
			...patterns,
		)
	);
}
...
export async function resolveSourceUri(line: string): Promise<string> {
	return <string>(
		await commands.executeJavaLanguageServerCommand(
			commands.JAVA_RESOLVE_SOURCE_URI,
			line,
		)
	);
}

export async function resolveInlineVariables(
	inlineParams: InlineParams,
): Promise<InlineVariable[]> {
	return <InlineVariable[]>(
		await commands.executeJavaLanguageServerCommand(
			commands.JAVA_RESOLVE_INLINE_VARIABLES,
			JSON.stringify(inlineParams),
		)
	);
}
