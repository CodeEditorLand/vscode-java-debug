 ...
 */
export async function getShortenApproachForCLI(
	config: vscode.DebugConfiguration,
	runtimeVersion: number,
): Promise<shortenApproach> {
	const recommendedShortenApproach =
		runtimeVersion <= 8
		...
		if (platformSettings && platformSettings.latestSupportedJavaVersion) {
			...
			if (latestSupportedVersion < runtimeVersion) {
				showWarningMessageWithTroubleshooting({
					message:
						'The compiled classes are not compatible with the runtime JDK. To mitigate the issue, please refer to "Learn More".',
 ...
 * @param runtimeVersion the target runtime version
 */
export async function addMoreHelpfulVMArgs(
	config: vscode.DebugConfiguration,
	runtimeVersion: number,
) {
	try {
		if (runtimeVersion >= 14) {
			...
			if (
				config.vmArgs &&
				config.vmArgs.indexOf(HELPFUL_NPE_VMARGS) >= 0
			) {
				return;
			}
...
async function shouldShortenIfNecessary(
	config: vscode.DebugConfiguration,
): Promise<boolean> {
	...
	if (!config.console || config.console === "internalConsole") {
		return (
			...
			classPathLength >= getMaxArgLength() ||
			modulePathLength >= getMaxArgLength()
		);
	} else {
...
function getMaxProcessCommandLineLength(
	config: vscode.DebugConfiguration,
): number {
...
function getEnvironmentLength(config: vscode.DebugConfiguration): number {
	...
	return _.isEmpty(env)
		? 0
		: Object.keys(env)
				.map((key) => strlen(key) + strlen(env[key]) + 1)
				.reduce((a, b) => a + b);
}
